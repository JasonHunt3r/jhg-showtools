...
final class MusicTests: XCTestCase {
    ...
    func testEditorStateRoundTripsAndAVersionNineLibraryGetsTheDefaults() throws {
        ...
        do {
            ...
            try db.exec("DROP TABLE group_items; DROP TABLE groups; ALTER TABLE collection_items DROP COLUMN sort_key; DROP TABLE rhythm_patterns; ALTER TABLE shows DROP COLUMN editor; PRAGMA user_version = 9;")
        }
    ...
    func testAVersionEightLibraryGetsNoMarkers() throws {
        ...
        do {
            let db = try Database(path: root.appendingPathComponent("Library.sqlite").path)
            try db.exec("DROP TABLE group_items; DROP TABLE groups; ALTER TABLE collection_items DROP COLUMN sort_key; DROP TABLE rhythm_patterns; ALTER TABLE shows DROP COLUMN editor; ALTER TABLE shows DROP COLUMN markers; PRAGMA user_version = 8;")
        }
        let lib = try Library(root: root)
        var show = try XCTUnwrap(lib.allShows().first)
        XCTAssertEqual(show.markers, [])
    ...
    func testAVersionSevenLibraryGetsNoMusic() throws {
        ...
        do {
            let db = try Database(path: root.appendingPathComponent("Library.sqlite").path)
            try db.exec("DROP TABLE group_items; DROP TABLE groups; ALTER TABLE collection_items DROP COLUMN sort_key; DROP TABLE rhythm_patterns; ALTER TABLE shows DROP COLUMN editor; ALTER TABLE shows DROP COLUMN markers; ALTER TABLE shows DROP COLUMN music; PRAGMA user_version = 7;")
        }
        let lib = try Library(root: root)
        var show = try XCTUnwrap(lib.allShows().first)
        XCTAssertEqual(show.music, [])
