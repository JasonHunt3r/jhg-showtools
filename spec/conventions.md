...
1. **The same action gives the same result wherever it's offered.** An
   area may offer fewer actions, never a different meaning for the same
   one. Edit Slides and Edit Show are the test case (audit §G).
...
| **⇧-click** | Select the range from the anchor (the last plain click or ⌘-click) to here, *replacing* the previous ⇧-range | — | Proposed. Today the grid and the timeline only ever add to it (B3, E1). Lists already do it right |
...
| **Double-click** | **Go into it:** open the thing one level deeper | See "Double-click and ⌥-click" below | **Open** |
| **⌥-click** | The alternative "go into it", or its second meaning | ⌥-click on a row handle already opens or closes every drawer | **Open** |
...
### Double-click and ⌥-click (Open)

Jason's rule: settle this by real testing. The candidates:
- **One meaning, "go into it"** (proposed as the frame): open the thing
  one level deeper.
  - a collection row: open its disclosure (Jason's original idea);
  - a Library tile: Quick Look, or Get Info (B6);
...
- **⌥-click** takes the other meaning where a thing has two. For example,
  if double-clicking a slide opens the inspector, ⌥-click opens the Slide
  Editor.
...
| **Return** | Do the default: OK in a dialog, commit a text field | On a selected item: rename it (Finder) | Built in dialogs; rename on Return is Proposed (B6, D4) |
| **Space** | Play and pause where there's a picture; Quick Look where there are files | Edit Slides has no picture: play from the selected slide, or nothing (G4) | Built in Edit Show and the player; Quick Look Proposed (B5) |
...
**One order, in every menu.** Groups are separated by a divider, and
