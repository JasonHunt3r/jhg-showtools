...
**Control Center tiles work unsigned** (measured 2026-09-22,
`tools/control-probe/`): an app with a WidgetKit extension, generated by
XcodeGen (`brew install xcodegen`) and built by Xcode 27, signed ad hoc
(no certificate on this Mac), installed in `~/Applications` and launched
once. Its two tiles, "Open BGTools" (a button) and "Desktop Show" (a
toggle), appeared in Control Center's gallery, could go in Control
Center or the menu bar, and the button opened the app's window (Jason).
The button's action ran in the sandboxed extension, not the app (the
app's log got nothing), so a tile that must *change* something in BGTools
